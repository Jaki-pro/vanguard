...
import { 
  Activity, ArrowRight, Play, CheckCircle, Globe, Shield, Zap, 
  BarChart3, Wifi, Cpu, MapPin, ChevronRight, Star, ExternalLink,
  Twitter, Linkedin, Github, Mail, Settings, Navigation, Signal,
  LogOut, LogIn
} from 'lucide-react';
...
const GradientText = ({ children, className = "" }: { children: React.ReactNode, className?: string }) => (
  <span className={`bg-clip-text text-transparent bg-gradient-to-r from-blue-400 via-indigo-400 to-emerald-400 ${className}`}>
    {children}
  </span>
);
...
const DecorativeChart = () => {
  return (
    <div className="flex items-end gap-1 h-16 w-full px-2">
      {[40, 65, 50, 80, 55, 90, 70, 85, 60, 95, 75, 50, 40, 60, 80].map((h, i) => (
        <motion.div
          key={i}
          initial={{ height: "20%" }}
          animate={{ height: `${h}%` }}
    ...
    <nav className="fixed top-0 left-0 right-0 z-50 px-6 py-4 backdrop-blur-xl border-b border-white/5">
      <div className="max-w-7xl mx-auto flex justify-between items-center">
        <div className="flex items-center gap-2">
          ...
          <span className="font-bold text-xl tracking-tight text-white">TrackFlow</span>
        </div>
  ...
  <section className="relative pt-40 pb-20 px-6 overflow-hidden">
    ...
    <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-16 items-center relative z-10">
      <div className="space-y-8">
        ...
        >
          Orchestrate your global logistics with the world's most advanced tracking OS. 
          Real-time telemetry, predictive insights, and automated compliance in one dashboard.
        </motion.p>
        ...
        >
          <Link href="/dashboard" className="group bg-blue-600 hover:bg-blue-500 text-white px-8 py-4 rounded-xl font-bold text-lg shadow-xl shadow-blue-600/20 transition-all flex items-center justify-center gap-2">
            Go to Dashboard
          ...
          <button className="group bg-slate-900/50 hover:bg-slate-800 border border-slate-700 text-white px-8 py-4 rounded-xl font-medium text-lg transition-all flex items-center justify-center gap-3 backdrop-blur-md">
            <Play size={18} className="fill-white group-hover:text-blue-400 transition-colors" /> Watch Demo
          </button>
        </motion.div>
        ...
        >
          ...
          <div className="text-sm text-slate-400">
            Trusted by <span className="text-white font-semibold">4,000+</span> logistics teams
          </div>
        </motion.div>
      </div>

      <motion.div 
        initial={{ opacity: 0, scale: 0.9, x: 20 }} animate={{ opacity: 1, scale: 1, x: 0 }} transition={{ duration: 0.8, delay: 0.2 }}
        className="relative"
      >
        ...
        <div className="relative bg-slate-900/80 backdrop-blur-xl border border-slate-700/50 rounded-3xl p-6 shadow-2xl overflow-hidden">
          <div className="flex justify-between items-center mb-8">
            <div>
              <div className="text-slate-400 text-xs font-bold uppercase tracking-wider mb-1">Live Fleet Status</div>
              <div className="text-2xl font-bold text-white flex items-center gap-2">
                1,248 <span className="text-emerald-400 text-sm font-normal bg-emerald-500/10 px-2 py-0.5 rounded-full border border-emerald-500/20">Active</span>
              </div>
            </div>
          ...
          <div className="relative h-48 bg-slate-800/50 rounded-2xl border border-slate-700/50 mb-6 overflow-hidden">
            <div className="absolute inset-0 opacity-30" style={{ backgroundImage: 'radial-gradient(#3b82f6 1px, transparent 1px)', backgroundSize: '20px 20px' }} />
            {[1,2,3].map(i => (
              <motion.div 
                key={i}
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-slate-800/30 p-4 rounded-xl border border-slate-700/50">
              ...
              <div className="text-xl font-bold text-white">94% <span className="text-xs font-normal text-emerald-400">+2%</span></div>
            ...
            <div className="bg-slate-800/30 p-4 rounded-xl border border-slate-700/50 relative overflow-hidden">
              ...
              <div className="text-xl font-bold text-white relative z-10">52 <span className="text-xs font-normal text-slate-500">mbps</span></div>
        ...
        >
          <div className="flex items-start gap-3">
            ...
            <div>
              <div className="text-xs font-bold text-white">Route Optimized</div>
              <div className="text-[10px] text-slate-400">Saved 45 mins • 12km</div>
            </div>
          </div>
        </motion.div>
      </motion.div>
  ...
  <section id="features" className="py-24 px-6 relative bg-slate-950">
    <div className="max-w-7xl mx-auto">
      <div className="text-center max-w-3xl mx-auto mb-20">
        <h2 className="text-3xl md:text-5xl font-bold text-white mb-6">Built for mission-critical logistics</h2>
        <p className="text-slate-400 text-lg">Scalable infrastructure that adapts to your fleet size, from a single delivery drone to a global maritime network.</p>
      </div>

      <div className="grid md:grid-cols-3 gap-6">
        <FeatureCard icon={Globe} title="Global Coverage" desc="Seamless connectivity switching between cellular (4G/5G) and satellite networks ensures zero-dead-zone tracking anywhere on Earth." delay={0} />
        <FeatureCard icon={Cpu} title="Edge Computing" desc="Process telemetry data directly on the device for sub-millisecond alerts on collisions, geofence breaches, or temperature spikes." delay={0.1} />
        <FeatureCard icon={Shield} title="Military-Grade Security" desc="SOC 2 Type II compliant platform with end-to-end AES-256 encryption for all data in transit and at rest." delay={0.2} />
        <FeatureCard icon={BarChart3} title="Predictive Analytics" desc="AI models analyze historical route data to predict delays, optimize fuel consumption, and schedule preventative maintenance." delay={0.3} />
        <FeatureCard icon={Wifi} title="Offline Sync" desc="Devices store data locally when disconnected and auto-sync immediately upon reconnection, ensuring 100% data integrity." delay={0.4} />
        <FeatureCard icon={Settings} title="Remote Diagnostics" desc="Troubleshoot issues, update firmware, and configure sensor parameters over-the-air (OTA) without recalling vehicles." delay={0.5} />
      </div>
    </div>
  </section>
);
      ...
      <div className="grid md:grid-cols-4 gap-12 mb-16">
        <div className="col-span-1 md:col-span-1">
          ...
          <p className="text-slate-400 text-sm leading-relaxed mb-6">
            The operating system for modern logistics. Track, analyze, and optimize your moving assets with unprecedented precision.
          </p>
        ...
        <div>
          ...
          <p className="text-slate-400 text-sm mb-4">Latest updates on features and releases.</p>
    ...
    <div className="bg-slate-950 min-h-screen">
      ...
      <section className="py-24 px-6 relative overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto text-center relative z-10">
          <h2 className="text-4xl md:text-5xl font-bold text-white mb-8">Ready to modernize your fleet?</h2>
          <p className="text-slate-400 text-lg mb-10 max-w-2xl mx-auto">
            Join thousands of companies using TrackFlow to save costs, improve safety, and deliver on time.
          </p>
